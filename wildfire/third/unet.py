...
class Unet(nn.Module):
    ...
    def __init__(
        ...
        encoder_channels = [info["num_chs"] for info in encoder.feature_info][
            ::-1
        ]
    ...
    def _preprocess_input(self, x, input_range=[0, 1], inplace=False):
        if not x.is_floating_point():
            raise TypeError(
                f"Input tensor should be a float tensor. Got {x.dtype}."
            )
...
class UnetDecoder(nn.Module):
    def __init__(
        ...
        for in_chs, out_chs in zip(in_channels, out_channels):
            self.blocks.append(
                DecoderBlock(in_chs, out_chs, norm_layer=norm_layer)
            )
...
if __name__ == "__main__":
    ...
    # print([m for m in timm.list_models() if "resnet" in m])
